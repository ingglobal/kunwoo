...
'mtr_total' : g5_1_material에서 조건 해당oop_idx, mtr_type='half', mtr_status NOT IN('delete','del','trash','cancel')의 총합계를 넘겨주세요.
...
'plt_cnt' : 절단동에 작업되고 있는 해당 oop_idx의 PLT 갯수
";
echo nl2br($data_str);
?>
</p>
    ...
    <div class="tbl_head02 tbl_wrap">
        <table>
            ...
            <tr class="<?php echo $bg.$tr_focus; ?>" orp_idx="<?php echo $row['orp_idx'] ?>" bom_idx="<?=$row['bom_idx']?>">
                ...
                <td class="td_mtr_total" total="<?=$row['mtr_total']?>"><?=number_format($row['mtr_total'])?></td>
        ...
        <caption>해당PLT의 상세재고목록(전체 : <?=$tcount?>개)</caption>
            <thead>
                <tr>
                    ...
                    <th scope="col">업체명<br><span>com_name</span></th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="t_com_idx"><?=$row['com_idx']?></td>
...
$('.btn_reg').on('click',function(){
    var mtr_total = $(this).parent().siblings('.td_mtr_total').attr('total');
    var heat = $(this).parent().siblings('.td_heat').find('select').val();
    var number = $(this).parent().siblings('.td_plt_num').find('input').val();
