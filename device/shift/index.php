...
if(!check_token1($getData[0]['token'])) {
    ...
    for($i=0;$i<sizeof($getData[0]['list']);$i++) {
        ...
        for($j=1;$j<=4;$j++) {
            // $arr['shf_range_'.$j.'_start'] = $arr['shf_range_'.$j.'_start'];
            // $arr['shf_range_'.$j.'_end'] = $arr['shf_range_'.$j.'_end'];
            if( preg_match("/[0-9]{2}:[0-9]{2}/",$arr['shf_range_'.$j.'_start']) ) {
                if( substr($arr['shf_range_'.$j.'_start'],-2) == 59 )
                    $arr['shf_range_'.$j.'_start'] .= ':59';
                else
                    $arr['shf_range_'.$j.'_start'] .= ':00';
            }
            if( preg_match("/[0-9]{2}:[0-9]{2}/",$arr['shf_range_'.$j.'_end']) ) {
                if( substr($arr['shf_range_'.$j.'_end'],-2) == 59 )
                    $arr['shf_range_'.$j.'_end'] .= ':59';
                else
                    $arr['shf_range_'.$j.'_end'] .= ':00';
            }
            
        }

        $arr['shf_range_1'] = $arr['shf_range_1_start'].'~'.$arr['shf_range_1_end'];
        $arr['shf_range_2'] = $arr['shf_range_2_start'].'~'.$arr['shf_range_2_end'];
        $arr['shf_range_3'] = $arr['shf_range_3_start'].'~'.$arr['shf_range_3_end'];
        $arr['shf_range_4'] = $arr['shf_range_4_start'].'~'.$arr['shf_range_4_end'];
        $arr['shf_status'] = 'ok';
        
        // 공통요소
        $sql_common[$i] = " com_idx = '".$arr['com_idx']."'
                        , mms_idx = '".$arr['mms_idx']."'
                        , shf_range_1 = '".$arr['shf_range_1']."'
                        , shf_range_2 = '".$arr['shf_range_2']."'
                        , shf_range_3 = '".$arr['shf_range_3']."'
                        , shf_range_4 = '".$arr['shf_range_4']."'
                        , shf_target_1 = '".$arr['shf_target_1']."'
                        , shf_target_2 = '".$arr['shf_target_2']."'
                        , shf_target_3 = '".$arr['shf_target_3']."'
                        , shf_target_4 = '".$arr['shf_target_4']."'
                        , shf_start_dt = '".$arr['shf_start_dt']."'
                        , shf_end_dt = '".$arr['shf_end_dt']."'
