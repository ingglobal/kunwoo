...
<div id="modal01" title="실행계획묶음등록" style="display:none;">
    <form name="form02" id="form02" autocomplete="off">
        ...
        <tr class="tr_flag">
            <td>
                <p style="padding:10px 0 6px;">절단설비선택</p>
        ...
        <tr class="tr_flag">
            <td>
                <p style="padding:10px 0 6px;">단조설비선택</p>
        ...
        <tr class="tr_flag">
            <td style="display:grid;grid-template-columns:5fr 5fr;">
                <div>
                ...
                <div>
                    <p style="padding:10px 0 6px;">생산종료일</p>
                    <input type="text" name="orp_done_date" id="orp_done_date" readonly class="frm_input readonly" style="width:100px;">
                </div>
            </td>
        </tr>
        <tr>
...
$( "#modal01" ).dialog({
    ...
    , close: function(event,ui){
        ...
        $('#form01').find('input[name="cut_mms_idx"]').remove();
        $('#form01').find('input[name="forge_mms_idx"]').remove();
        $('#form01').find('input[name="mb_id"]').remove();
        $('#form01').find('input[name="orp_start_date"]').remove();
        $('#form01').find('input[name="orp_done_date"]').remove();
...
function form02_submit(f) {
    ...
    var cut_mms_idx = f.cut_mms_idx.value;
    var forge_mms_idx = f.forge_mms_idx.value;
    var mb_id = f.mb_id.value;
    var mb_name = f.mb_name.value;
    var orp_start_date = f.orp_start_date.value;
    var orp_done_date = f.orp_done_date.value;
    ...
    if(!confirm("대량데이터는 시간이 1분이상 소요될수 있습니다.\n실행하는 동안 창을 닫거나, 다른버튼을 클릭해서는 안됩니다.\n작업을 진행하시겠습니까?")){
        return false;
    }
    ...
    $('<input type="hidden" name="cut_mms_idx" value="'+cut_mms_idx+'">').insertBefore(tkn_obj);
    $('<input type="hidden" name="forge_mms_idx" value="'+forge_mms_idx+'">').insertBefore(tkn_obj);
    ...
    $('<input type="hidden" name="orp_done_date" value="'+orp_done_date+'">').insertBefore(tkn_obj);
