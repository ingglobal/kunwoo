...
$html_title = ($w=='')?'추가':'수정'; 
$g5['title'] = $mms['mms_name'].' 부속품 '.$html_title;
include_once('./_head.sub.php');

// 각 항목명 및 항목 설정값 정의, 형식: 항목명, required, 폭, 단위(개, 개월, 시, 분..), 설명, tr숨김
$items1 = array(
    "mms_idx"=>array("설비명","required",0,0,'','none')
    ,"mmp_name"=>array("부속품명","required",0,0,'')
    ,"mmp_size"=>array("규격","required",0,0,'ex) 650W*350H 또는 3HP 380V 3P 등과 같이 입력')
    ,"mmp_price"=>array("가격","",80,'원','')
    ,"mmp_install_date"=>array("설치일","required",80,0,'')
    ,"mmp_last_date"=>array("최종교체일","",80,0,'')
    ,"mmp_change_cycle"=>array("교체주기(일)","required",60,'일','일수로 입력하세요. 365=1년, 730=2년')
    ,"mmp_memo"=>array("메모","",0,0,'')
);
?>
<div class="new_win">
