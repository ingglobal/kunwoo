...
$items1 = array(
    ...
    ,"cod_idx"=>array("코드고유번호","required",60,0,'','',2)
    ,"trm_idx_category"=>array("분류","required",60,0,'','',0)
    ,"dta_shf_no"=>array("교대번호","required",30,'번','','',2)
    ,"dta_shf_max"=>array("총교대수","required",30,'개','',0)
    ,"dta_group"=>array("데이터그룹","required",0,0,'err=에러,pre=예지,run=가동시간,product=생산,mea=측정','',2)
    ,"dta_code"=>array("에러코드","required",0,0,'각 현장의 설비마다 코드의 역할이 다른 수 있습니다.','')
    ,"dta_message"=>array("메시지",'',350,'','메시지는 설비장비 동기화 이슈가 있어서 저장하고 관리합니다.','',2)
    ,"dta_dt"=>array("일시","required",100,0,'일시는 timestamp값입니다.','',0)
    ,"dta_status"=>array("상태","required",60,'','0=정상, 1=비정상','',2)
);
?>
