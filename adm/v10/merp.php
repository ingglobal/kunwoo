...
$page_key = ($sub_page) ? 'merp_'.$sub_page : 'merp';
...
$head_page_path = (G5_IS_MOBILE) ?  G5_USER_ADMIN_MOBILE_PATH.'/merp.head.php' : G5_USER_ADMIN_PATH.'/merp.head.php';


//$sub_page변수가 잘못 넘어왔을때
if($sub_page && !is_file(G5_USER_ADMIN_PATH.'/merp.'.$sub_page.'.php')) {
    alert('존재하지 않는 페이지 입니다.');
} else if($sub_page && is_file(G5_USER_ADMIN_PATH.'/merp.'.$sub_page.'.php')) {
    ;
} else {
    $sub_page = 'main';
}
...
$g5['title'] = 'merp '.$ttl_arr[$sub_page].' 보고서';
...
add_stylesheet('<link rel="stylesheet" href="'.((G5_IS_MOBILE) ? G5_USER_ADMIN_MOBILE_URL : G5_USER_ADMIN_URL).'/css/merp1.css">', 0);
...
function frame_loading(flag) {
	...
	$('.tab_content').find('iframe').attr('src', './merp.empty.php');
	...
	$('#frame_'+frame_file).attr('src', './merp.'+frame_file+'.php?'+data_serialized);
}
