...
// 원가 테이블 계산
$unit_days = array('consumable','electricity','oil');   // 소모품, 전기세, 유류대 등은 월간 비용이므로 일간으로 나누어 주어야 함
$sql = "SELECT mms_idx, csc_idx, csc_type, csc_ym, csc_price
        FROM {$g5['cost_config_table']}
        WHERE com_idx = '".$_SESSION['ss_com_idx']."'
            AND csc_status IN ('ok')
            AND csc_ym >= '".substr($st_date,0,7)."-00'
            AND csc_ym <= '".substr($en_date,0,7)."-00'
            AND mms_idx IN (".$ser_mms_idx.",0)
        ORDER BY mms_idx DESC, csc_type, csc_ym
";
// echo $sql.'<br>';
$rs = sql_query($sql,1);
$byunit = 86400;
for($i=0;$row=sql_fetch_array($rs);$i++){
    ...
    $ts1 = strtotime($st_date);    // 시작 timestamp
    $ts2 = strtotime($en_date);    // 종료 timestamp
    // echo date("Y-m-d",$ts1).'~'.date("Y-m-d",$ts2).'<br>';
    for($k=$ts1;$k<=$ts2;$k+=$byunit) {
        $date1 = preg_replace("/[ :-]/","",date("Y-m-d",$k));   // 날짜중에서 일자 추출하여 배열키값으로!
        $day_count1 = date('t',$k);   // 월의 마지막일(총날수)
        // echo $date1.'(오늘날짜) / 월총날짜:'.$day_count1.'<br>';
        // 월간인 경우는 일간으로 나누어 주어야 함
        if(in_array($row['csc_type'],$unit_days)) {
            $cost[$row['csc_type']][$date1] = $row['csc_price']/$day_count1;
        }
        else {
            $cost[$row['csc_type']][$date1] = $row['csc_price'];
        }
        // echo '------<br>';
    }
}
...
<div class="local_desc01 local_desc" style="display:no ne;">
    ...
    <p>생산루베(m<sup>3</sup>) = 생산수량*0.18</p>
    <p>일생산원가 = 하루원가총비용(전기세+소모품비+유류대+추가작업비용) * 실가동시간/24</p>
    <p>시간당루베단위생산원가 = 시간당생산원가 / 시간당루베생산량</p>
</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        <th scope="col">날짜</th>
        <th scope="col">기종</th>
        <th scope="col" style="display:none;">품명</th>
        <th scope="col">전기세</th>
        <th scope="col">소모품비</th>
        <th scope="col">장비유류대</th>
        <th scope="col">생산수량</th>
        <th scope="col">생산루베(m<sup>3</sup>)</th>
        <th scope="col">작업시간(분)</th>
        <th scope="col">공제(분)</th>
        <th scope="col">실가동분합계(시)</th>
        <th scope="col">일생산원가</th>
        <th scope="col">추가작업비용</th>
        <th scope="col">일생산원가합계</th>
        <th scope="col" style="display:none;">비가동시간(시)</th>
        <th scope="col">시간당루베단위생산원가</th>
    </tr>
    </thead>
    <tbody>
    <?php
    ...
    {
        $row['dta_date1'] = preg_replace("/-/","",$row['dta_date']);
        ...
        // 추가작업비용(현장작업자인건비 + 장비기사인건비)
        $cost['labor'][$row['dta_date1']] = $cost['worker'][$row['dta_date1']] + $cost['engineer'][$row['dta_date1']];

        // 하루원가총비용(인건비 제외)
        $row['cost_price_day'] = $cost['electricity'][$row['dta_date1']] + $cost['consumable'][$row['dta_date1']]
                                    + $cost['oil'][$row['dta_date1']];
        // $row['cost_price_day'] = $cost['electricity'][$row['dta_date1']] + $cost['consumable'][$row['dta_date1']]
        //                             + $cost['oil'][$row['dta_date1']] + $cost['labor'][$row['dta_date1']];

        // 일생산원가 (전기세를 하루종일 쓴 게 아니고 운영시간만큼만 사용했다고 봄)
        $row['cost_price_time_working'] = $row['cost_price_day']*$row['workhour']/24;

        // 일생산원가 합계 += 인건비
        $row['cost_price_day_total'] = $row['cost_price_time_working'] + $cost['labor'][$row['dta_date1']];

        // 시간당생산원가 (운영시간으로 한번 더 나누어줘야 함)
        $row['cost_price_time'] = $row['cost_price_time_working']/$row['workhour'];

        // 시간당루베생산량
        $row['output_lube_time'] = $row['output_sum_lube']/$row['workhour'];

        // 시간당단위루베당원가 = 시간당생산원가/시간당루베생산량
        // $row['cost_price_time_working_by_lube'] = $row['cost_price_time_working']/$row['output_lube_time'];
        $row['cost_price_time_working_by_lube'] = $row['cost_price_day_total']/$row['output_lube_time'];
    ...
    <tr class="<?php echo $bg; ?> tr_<?=$row['dmn_status']?>">
        ...
        <td style="display:none;"><?=$row['ahref'].$mmi_name[$row['mms_idx']][$row['dta_mmi_no']]?></a></td><!-- 품명 -->
        <td class="td_right"><?=number_format($cost['electricity'][$row['dta_date1']])?></td><!-- 전기세 -->
        <td class="td_right"><?=number_format($cost['consumable'][$row['dta_date1']])?></td><!-- 소모품비 -->
        <td class="td_right"><?=number_format($cost['oil'][$row['dta_date1']])?></td><!-- 장비유류대 -->
        ...
        <td class="td_right"><span style="font-size:.8em;">(<?=substr($row['period']['dta_ymdhis_min'],11)?>~<?=substr($row['period']['dta_ymdhis_max_display'],11)?>)</span> <?=$row['workmin']?></td><!-- 작업시간(분) -->
        <td class="td_right"><?=$row['offworkmin']?></td><!-- 공제(분) -->
        <td><?=$row['workrealmin']?> (<?=$row['workhour']?>)</td><!-- 실작업시간(시) -->
        <td class="td_right"><?=number_format($row['cost_price_time_working'])?></td><!-- 일생산원가 -->
        <td class="td_right"><?=number_format($cost['labor'][$row['dta_date1']])?></td><!-- 추가작업비용 -->
        <td class="td_right"><?=number_format($row['cost_price_day_total'])?></td><!-- 일생산원가합계 -->
        <td style="display:none;"><?=$row['downtimemin']?> (<?=$row['downtimehour']?>)</td><!-- 비가동시간(시) -->
        <td class="td_right"><?=number_format($row['cost_price_time_working_by_lube'])?> <span style="font-size:0.8em;">원/m<sup>3</sup></span></td><!-- 시간당생산원가 -->
    </tr>
    <?php
    }
    if ($i == 0)
