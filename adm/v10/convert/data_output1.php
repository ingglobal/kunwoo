<?php
// 실행주소: http://local.ingsystem.com/adm/v10/convert/data_output1.php
...
//-- 설정값
$group_array = array('product','product','product');
$item_array = array(0,0,0,0,0,1,1,1,2);   // 기종번호
$defect_array = array(0,0,0,1,0,0,0,1,0,0,0,1);
$defect_type_array = array(1,1,1,2,2,3,3,3,4,4,4,5);
$message_array = array('불량','형상불량','치수불량','스크레치불량','사출불량','기타불량');
...
//		break;
	...
	$dta_value[$cnt] = rand(1,10);
    $dta_defect[$cnt] = $defect_array[rand(0,sizeof($defect_array)-1)];
    if($dta_defect[$cnt]) {
        $dta_defect_type[$cnt] = $defect_type_array[rand(0,sizeof($defect_type_array)-1)];
        $dta_message[$cnt] = $message_array[$dta_defect_type[$cnt]];
    }
	
	$sql1 = " INSERT INTO g5_1_data_output SET								      ".
			"	com_idx	= '".rand(1,67)."'							              ".
			"	, imp_idx	= '".rand(1,16)."'							          ".
			"	, mms_idx	= '".rand(1,4)."'							          ".
			"	, shf_idx	= '".rand(1,4)."'							          ".
			...
			"	, dta_defect = '".$dta_defect[$cnt]."'                            ".
			"	, dta_defect_type = '".$dta_defect_type[$cnt]."'                  ".
			"	, dta_dt = '".$dta_dt[$cnt]."'								      ".
    ...
    echo "<script> document.all.cont.innerHTML += '".$cnt.". ".$dta_group[$cnt]." = ".$dta_value[$cnt]." ".$dta_message[$cnt]." <br>'; </script>".PHP_EOL;
...
$sql = "INSERT INTO g5_1_data_output_sum (com_idx, imp_idx, mms_idx, shf_idx, dta_shf_no, dta_mmi_no, dta_group, dta_defect, dta_defect_type, dta_message, dta_date, dta_value)
        ...
        WHERE dta_status = 0
        GROUP BY mms_idx, dta_shf_no, dta_mmi_no, dta_defect, dta_defect_type, dta_date
