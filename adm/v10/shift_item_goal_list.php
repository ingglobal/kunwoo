...
$sql_common = " FROM {$g5_table_name} AS ".$pre." 
                LEFT JOIN {$g5['shift_table']} AS shf ON shf.shf_idx = ".$pre.".shf_idx
                LEFT JOIN {$g5['mms_item_table']} AS mmi ON mmi.mmi_idx = ".$pre.".mmi_idx
...
$sql = " SELECT SQL_CALC_FOUND_ROWS DISTINCT ".$pre.".*
            , shf.com_idx AS com_idx
            , shf.mms_idx AS mms_idx
            , shf_start_dt
            , shf_end_dt
            , mmi_name
            , mmi_no
...
$items1 = array(
    "sig_idx"=>array("번호",0,0,0)
    ,"mms_name"=>array("설비명",0,0,0)
    ,"sig_shf_no"=>array("교대번호",0,0,0)
    ,"shf_apply_period"=>array("적용기간",0,0,0)
    ,"mmi_no"=>array("기종번호",0,0,0)
    ,"sig_item_target"=>array("목표수량",0,0,0)
);
?>
<style>
.td_shf_apply_period {font-size:0.8em;}
.td_mms_name {text-align:left !important;}
.td_mmi_no {text-align:left !important;}
</style>
...
<select name="sfl" id="sfl">
    ...
    <?php
    $skips = array('sig_idx','mms_name','sig_item_target');
    if(is_array($items1)) {
    ...
    <option value="shf.com_idx" <?=get_selected($sfl, "shf.com_idx")?>>업체번호</option>
    <?php } ?>
</select>
        ...
        //    print_r2($items1);
            foreach($items1 as $k1 => $v1) {
                ...
                if(preg_match("/_price$/",$k1)) {
                    ...
                    $list[$k1] = $row[$k1].'  <span class="font_size_8">'.cut_str($row['mmi_name'],17,'..').'</span>';
                }
                else if($k1=='sig_item_target') {
                    $list[$k1] = number_format($row[$k1]);
                }
                else if($k1=='shf_apply_period') {
                    $list[$k1] = $row['shf_start_dt'].' ~ '.$row['shf_end_dt'];
                }
