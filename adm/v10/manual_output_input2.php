...
<div class="div_shift_info">
    ...
    <div class="btns">
		<a href="javascript:" class="btn" flag="down">전체열기 <i class="fa fa-chevron-down"></i></a>
	</div>
</div>
		...
		';
			...
			for($j=0;$j<sizeof($mms_mmi[$shift[$i]['mms_idx']]);$j++) {
				...
				echo '<div class="td_inputbox '.$item_class[$j].'" style="display:'.$item_display[$j].'" item="'.$item_no.'">'
						.'<input type="text" name="chk[]" value="'.$idx.'">'
						.'<input type="text" name="mms_idx['.$idx.']" value="'.$shift[$i]['mms_idx'].'">'
						.'<input type="text" name="shift_no['.$idx.']" value="'.$shift_target[$i].'">'
						.'<input type="text" name="item_no['.$idx.']" value="'.$mms_mmi[$shift[$i]['mms_idx']][$j].'">'
						.'<input type="text" name="dta_time['.$idx.']" value="'.$shift_time[$shift_target[$i]]['end'].'">'
					...
					if(is_array($mms_status[$shift[$i]['mms_idx']])) {
						foreach($mms_status[$shift[$i]['mms_idx']] as $k1=>$v1) {
							...
							echo '<div class="item_each">'.$v1.': <input name="dta_time['.$idx.']['.$k1.']" class="frm_input input_defect" value="'.$output2[$i][$j].'"></div>';
							// echo '<div class="item_each">'.$v1.': <input name="'.$input_name.'" class="frm_input input_defect" value="'.$output2[$i][$j].'"></div>';
						}
					}
					echo '<div class="item_each_add" item="'.$item_no.'" output="'.$output[$i][$j].'" time="'.$shift_time[$shift_target[$i]]['end'].'">추가...</div>';
					echo '</div>';
...
$(document).on('click','.item_each_add',function(e){
	e.preventDefault();
	var href = './manual_output_add_form.php?item='+$(this).attr('item')+'&output='+$(this).attr('output')+'&time='+$(this).attr('time');
