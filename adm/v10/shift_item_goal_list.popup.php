...
if(!$shf_idx)
    alert_close('교대 정보가 존재하지 않습니다.');
$shf = get_table_meta('shift','shf_idx',$shf_idx);
$mms = get_table_meta('mms','mms_idx',$shf['mms_idx']);
$com = get_table_meta('company','com_idx',$shf['com_idx']);
...
$g5['title'] = $mms['mms_name'].' 부속품 ('.number_format($total_count).')';
include_once('./_head.sub.php');

// 각 항목명 및 항목 설정값 정의, 형식: 항목명, colspan, rowspan, 정렬링크여부(타이틀클릭)
$items1 = array(
    "mmp_name"=>array("부속품명",0,0,0)
    ,"mmp_size"=>array("규격",0,0,0)
    ,"mmp_install_date"=>array("설치일",0,0,0)
    ,"mmp_change_cycle"=>array("교체주기",0,0,0)
);
?>
<!-- 체크박스 전체 선택 -->
