...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    $list[$i]['num'] = $num;
    $list[$i]['opener_href'] = G5_USER_ADMIN_URL.'/bbs_board.php?bo_table='.$row['bo_table'];//get_pretty_url($row['bo_table']);
    $list[$i]['opener_href_wr_id'] = G5_USER_ADMIN_URL.'/bbs_board.php?bo_table='.$row['bo_table'].'&amp;wr_id='.$row['wr_id'];
    $list[$i]['bo_subject'] = $row2['bo_subject'];
    $list[$i]['subject'] = $subject;
    $list[$i]['del_href'] = './bbs_scrap_delete.php?ms_id='.$row['ms_id'].'&amp;page='.$page;
}
