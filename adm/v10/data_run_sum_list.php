...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $row['mms'] = sql_fetch(" SELECT mms_name, mms_set_output, mms_set_error FROM {$g5['mms_table']} WHERE mms_idx = '".$row['mms_idx']."' ");
        ...
        $sql1 = "SELECT SQL_NO_CACHE SUM(dta_value) AS dta_value_sum
        ...
        // 설비가 교대 기준: 날짜가 넘어가도 교대번호가 2이상이면서 오전 등록인 건수는 오늘날짜로 합산해야 함 
        if($row['mms']['mms_set_output']=='shift') {
            // echo '생산: shift<br>';

        }


        $row['dta_value_sum'] = sectohis($sum1['dta_value_sum']);
...
//                    $list[$k1] = substr($row[$k1],0,10);
                    ...
                    $list[$k1] = sectohis($row[$k1]);
                }
                else if($k1=='dta_value_sum') {
                    $row['dta_value_sum_color'] = ($list['dta_value']!=$row[$k1]) ? 'darkorange':'';
