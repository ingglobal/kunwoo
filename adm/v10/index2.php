...
<ul class="list_wrapper">
    ...
    <li class="ui-state-default mms_container <?=$img_container_style?>"
        ...
        <tr class="tr_title">
            <td colspan="2" class="td_center"><?=cut_str($row['mms_name'],7,'..')?></td>
        </tr>
        <tr>
            <td>생산기종</td>
        ...
        <tr>
            <td colspan="2" class="td_dot" style="">
    ...
    <script>
    $(function(e) {
        var mms_status_colors = ['#eebb0c','#62ee0c','#ee0c0c'];
            ...
            $.ajax({
                ...
                dataType:'json', timeout:15000, beforeSend:function(){}, success:function(res){
                    ...
                    if(res.result == true) {
                        ...
                        // run status related.
                        // if run[0] exists, it will positioned an the first dot.
                        // and all the other dots will be shifted by one positon. The last dot dispears.
                        if(res.run) {

                            // console.log(res.run[0]);
                            if( res.run[0] != undefined && res.run[0]!='' ) {
                                // console.log('res.run[0] exists. '+res.run[0]);
                                // The last dot disappers.
                                my_mms_li.find('.td_dot i:last-child').remove();
                                // the first will be stationed.
                                if(res.run[0]=='') {
                                    my_mms_li.find('.td_dot').prepend('<i class="fa fa-circle-o"></i>');
                                }
                                else {
                                    my_mms_li.find('.td_dot').prepend('<i class="fa fa-circle" style="color:'+mms_status_colors[res.run[0]]+'"></i>');
                                }
                            }
                            // if run[1~5] fetched, it will change the status of existing dots.
                            else {
                                $.each(res.run, function(i,v){
                                    if(v=='') {
                                        my_mms_li.find('.td_dot i:eq('+(i-1)+')').attr('class','fa fa-circle-o');
                                        // if( my_mms_li.attr('mms_idx') == 7 ) {
                                        //     console.log(i+'. '+v);
                                        // }
                                    }
                                    else {
                                        my_mms_li.find('.td_dot i:eq('+(i-1)+')').attr('class','fa fa-circle').css('color',mms_status_colors[v]);
                                        // if( my_mms_li.attr('mms_idx') == 7 ) {
                                        //     console.log(i+'. '+v);
                                        // }
                                    }
