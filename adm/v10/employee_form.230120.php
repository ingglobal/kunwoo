...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr style="display:<?php if(!$member['mb_manager_yn']) echo 'none';?>;">
        ...
        <th scope="row"><label for="mb_7">부서선택<strong class="sound_only">필수</strong></label></th>
        <td>
            <select name="mb_7" id="mb_7_parts">
                <?=$g5['set_mb_parts_value_options']?>
            </select>
            <script>
                $('#mb_7_parts').val('<?=$mb['mb_7']?>');
            </script>
        </td>
    </tr>
...
function fmember_submit(f)
{
    ...
    if(f.w.value == 'u' && f.mb_password.value){
        ...
        var pt = /^(?=.*[0-9]+)[a-zA-Z][a-zA-Z0-9]{7,}$/;
        if(!pt.test(pw)){
            alert('비밀번호가 영문숫자조합 및 8글자이상의 문자열\n규칙에 위반된 구성입니다.');
            f.mb_password.focus();
            return false;
        }
    }
