...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    if (!strstr($row['wr_option'], 'secret') ||
        ...
        if(!get_session($ss_name))
            $list[$i]['content'] = '<a href="'.G5_USER_ADMIN_URL.'/bbs_password.php?w=sc&amp;bo_table='.$bo_table.'&amp;wr_id='.$list[$i]['wr_id'].'&amp;'.$qstr.'" class="s_cmt">댓글내용 확인</a>';
        else {
            $list[$i]['content'] = conv_content($row['wr_content'], 0, 'wr_content');
            $list[$i]['content'] = search_font($stx, $list[$i]['content']);
        }
    ...
    if ($is_comment_write || $is_admin)
    {
        ...
        if ($member['mb_id'])
        {
            if ($row['mb_id'] === $member['mb_id'] || $is_admin)
            {
                ...
                $list[$i]['del_link']  = G5_USER_ADMIN_URL.'/bbs_delete_comment.php?bo_table='.$bo_table.'&amp;comment_id='.$row['wr_id'].'&amp;token='.$token.'&amp;page='.$page.'&amp;'.$qstr;
                $list[$i]['is_edit']   = true;
                $list[$i]['is_del']    = true;
            }
        }
        ...
        {
            if (!$row['mb_id']) {
                $list[$i]['del_link'] = G5_USER_ADMIN_URL.'/bbs_password.php?w=x&amp;bo_table='.$bo_table.'&amp;comment_id='.$row['wr_id'].'&amp;page='.$page.'&amp;'.$qstr;
                $list[$i]['is_del']   = true;
            }
        }
...
$comment_action_url = https_url(G5_ADMIN_DIR.'/'.G5_USER_ADMIN_DIR)."/bbs_write_comment_update.php";
$comment_common_url = short_url_clean(G5_USER_ADMIN_URL.'/bbs_board.php?'.clean_query_string($_SERVER['QUERY_STRING']));
