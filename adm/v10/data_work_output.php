...
for($i = 0 ; $row = sql_fetch_array($rs); $i++) {
    ...
    $table_name = 'g5_1_data_output_'.$row['mms_idx'];
    $sql = "SELECT EXISTS (
    ...
    $tb1 = sql_fetch($sql,1);
    if(!$tb1['flag']) {
        $file = file('../../device/output/sql_write.sql');
    ...
    $sql = "INSERT INTO {$table_name} (dta_shf_no, dta_mmi_no, dta_defect, dta_defect_type, dta_dt, dta_date, dta_value, dta_reg_dt, dta_update_dt)
            SELECT dta_shf_no, dta_mmi_no, dta_defect, dta_defect_type, dta_dt, dta_date, dta_value, dta_reg_dt, dta_update_dt
            FROM g5_1_data_output
            WHERE mms_idx = '".$row['mms_idx']."' 
                AND dta_group = 'product' 
                AND dta_status = 0 
                {$sql_dta_dt}
            ORDER BY dta_idx
    ";
