...
$error_exist_sql = " DESC `{$tbl_error}` ";
$error_chk = @sql_query($error_exist_sql);
if(!$error_chk){
    include(G5_USER_CRON_PATH.'/icmms/tbl_create_error.php');
}
$mta_chk = sql_fetch(" SELECT mta_key FROM {$g5['meta_table']} 
                    WHERE mta_db_table = 'icmms_error'
                        AND mta_db_id = '{$mms_idx}'
                        AND mta_key = '{$tbl_error}' ");
if(!$mta_chk['mta_key']){
    meta_update(array("mta_country"=>"ko_KR","mta_db_table"=>"icmms_error","mta_db_id"=>$mms_idx,"mta_key"=>$tbl_error,"mta_value"=>'0',"mta_reg_dt"=>G5_TIME_YMDHIS));
}
...
foreach($res as $row){
    ...
    $ins_sql = " INSERT INTO {$tbl_error} SET
            com_idx = '{$row['com_idx']}'
            , imp_idx = '{$row['imp_idx']}'
            , mms_idx = '{$row['mms_idx']}'
            , shf_idx = '{$row['shf_idx']}'
            ...
            , dta_shf_max = '{$row['dta_shf_max']}'
            , dta_group = '{$row['dta_group']}'
            , dta_code = '{$row['dta_code']}'
            , dta_dt = '{$row['dta_dt']}'
            , dta_message = '{$row['dta_message']}'
            , dta_status = '{$row['dta_status']}'
