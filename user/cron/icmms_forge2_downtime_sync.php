...
$downtime_exist_sql = " DESC `{$tbl_downtime}` ";
$downtime_chk = @sql_query($downtime_exist_sql);
if(!$downtime_chk){
    include(G5_USER_CRON_PATH.'/icmms/tbl_create_downtime.php');
}
$mta_chk = sql_fetch(" SELECT mta_key FROM {$g5['meta_table']} 
                    WHERE mta_db_table = 'icmms_downtime'
                        AND mta_db_id = '{$mms_idx}'
                        AND mta_key = '{$tbl_downtime}' ");
if(!$mta_chk['mta_key']){
    meta_update(array("mta_country"=>"ko_KR","mta_db_table"=>"icmms_downtime","mta_db_id"=>$mms_idx,"mta_key"=>$tbl_downtime,"mta_value"=>'0',"mta_reg_dt"=>G5_TIME_YMDHIS));
}
...
foreach($res as $row){
    ...
    $ins_sql = " INSERT INTO {$tbl_downtime} SET
            com_idx = '{$row['com_idx']}'
            , imp_idx = '{$row['imp_idx']}'
            , mms_idx = '{$row['mms_idx']}'
            , mmg_idx = '{$row['mmg_idx']}'
            , mst_idx = '{$row['mst_idx']}'
            , dta_start_dt = '{$row['dta_start_dt']}'
            , dta_end_dt = '{$row['dta_end_dt']}'
            , dta_memo = '{$row['dta_memo']}'
...
meta_update2(array("mta_country"=>"ko_KR","mta_db_table"=>"icmms_downtime","mta_db_id"=>$mms_idx,"mta_key"=>$tbl_downtime,"mta_value"=>$last_idx,"mta_reg_dt"=>G5_TIME_YMDHIS));
